			...
			case 3: // locação de veículo
				if (listCliente.size() == 0){
					...
					for(int x = 0; x < listCliente.size(); x++){	
						...
						System.out.printf("  "+x+1 + " " + cliente.getNome(), cliente.getEnd(), cliente.getTelefone(), cliente.getIdade(), cliente.getRg(), cliente.getCpf());
					}
					while(true) {
				...
				while(true) {
					new Clear().clearConsole();
					System.out.printf("\n  Locação de Veí	c                 leitor.nextLine();\n" + 
